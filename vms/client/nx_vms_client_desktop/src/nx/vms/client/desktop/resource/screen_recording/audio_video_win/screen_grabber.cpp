...
HRESULT ScreenGrabber::InitD3D(HWND hWnd)
{
...
void ScreenGrabber::allocateTmpFrame(int width, int height, AVPixelFormat format)
{
...
void ScreenGrabber::captureFrameOpenGL(CaptureInfoPtr data)
{
...
CaptureInfoPtr ScreenGrabber::captureFrame()
{
...
void ScreenGrabber::drawLogo(quint8* data, int width, int height)
{
    if (m_logo.width() == 0)
        return;
...
bool ScreenGrabber::direct3DDataToFrame(void* opaque, AVFrame* pFrame)
{
...
bool ScreenGrabber::dataToFrame(
...
bool ScreenGrabber::capturedDataToFrame(CaptureInfoPtr captureInfo, AVFrame* pFrame)
{
...
int ScreenGrabber::width() const
{
    return m_outWidth;
}

int ScreenGrabber::height() const
{
    return m_outHeight;
}
...
int ScreenGrabber::screenWidth() const
{
    return m_ddm.Width;
}

int ScreenGrabber::screenHeight() const
{
    return m_ddm.Height;
}

void ScreenGrabber::pleaseStop()
{
    NX_MUTEX_LOCKER lock(&m_guiWaitMutex);
    m_needStop = true;
    m_waitCond.wakeAll();
