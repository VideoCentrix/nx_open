    ...
    QnSessionAwareDelegate(parent)
{
    ...
    connect(action(action::ConvertLayoutToSharedAction), &QAction::triggered, this,
    ...
    if (NX_ASSERT(cachingController))
    {
        connect(cachingController, &CachingAccessController::permissionsChanged, this,
            [this](const QnResourcePtr& resource)
            {
                ...
                if (!accessController->hasPermissions(layoutResource, Qn::ReadPermission))
                {
...
void LayoutsHandler::convertLayoutToShared(const LayoutResourcePtr& layout)
{
